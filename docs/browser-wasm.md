...
npm install
npm run build:wasm   # dotnet publish + copy public/docx-sax/_framework
npm run build        # Next/Vercel build only
npm run test:e2e     # Playwright upload -> WASM load -> parse -> render smoke
```

Use `npm test` in the demo directory to run all three steps locally. For Vercel, keep `npm run build` as the build command and feed `public/docx-sax/_framework` from CI/package prep; otherwise Vercel will pay the full .NET browser publish cost on every build.
...
`npm run test` publishes the .NET browser WASM project, starts a Vite server, loads the bridge in Chromium, fetches the public/generated `simple.docx` fixture, and verifies package/part/element/text/end events are observed.

CI runs the same browser smoke on the Linux leg. It is intentionally Linux-only for now because Playwright browser installation is the heaviest new dependency and the Native wrapper is also Linux-first.
...
- The browser package excludes PDB, Brotli, and gzip sidecars from npm. The exact packed size depends on the current scoped-package split, but the browser runtime payload is now the trimmed raw asset set rather than the earlier 46 MiB untrimmed publish.
